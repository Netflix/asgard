...
<g:if test="${checkHealth}">
  <tr class="prop" title="Wait for instance to register with Discovery and then wait for it to pass its health check">
    ...
    <td class="value"><g:checkBox name="checkHealth" value="${checkHealth}" checked="${checkHealth}"/>
      <label for="checkHealth">Wait for Discovery health check pass</label></td>
  </tr>
</g:if>
<tr class="prop waitTime" style="${checkHealth ? 'display:none;' : ''}" title="Additional time to wait after OS boot when not waiting for Discovery health check">
