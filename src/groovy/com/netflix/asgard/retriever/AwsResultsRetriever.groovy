...
abstract class AwsResultsRetriever<T, Q, S> {
    ...
    AwsResultsRetriever(int maxResults = Integer.MAX_VALUE, int throttleTimeBetweenRequests = 20) {
        this.maxResults = maxResults
        this.throttleTimeBetweenRequests = throttleTimeBetweenRequests
    }

    List<T> retrieve(Region region, Q request) {
        List<T> items = []
        Integer remaining = maxResults
        String nextToken = null
        while (remaining > 0) {
            if (nextToken) { Time.sleepCancellably(throttleTimeBetweenRequests) } // Avoid rate limiting
            setNextToken(request, nextToken)
            limitRetrieval(request, remaining)
            S result = makeRequest(region, request)
    ...
    protected abstract S makeRequest(Region region, Q request)
