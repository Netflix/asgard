...
class DefaultAdvancedUserDataProviderSpec extends Specification {
    ...
    def 'should delegate to original user data provider implementation registered as a plugin'() {
        ...
        launchContext.autoScalingGroup = new AutoScalingGroup(autoScalingGroupName: 'go-ahead')
        launchContext.launchConfiguration = new LaunchConfiguration(launchConfigurationName: 'go-ahead-123')

        when:
        String userData = provider.buildUserDataForCloudObjects(launchContext)
