...
class LocalFilesCredentialsProviderSpec extends Specification {
    ...
    LocalFilesCredentialsProvider provider
    ...
    def 'should get credentials from local files if properly configured, and cache them for subsequent requests'() {
        provider = new LocalFilesCredentialsProvider(configService, localFileReader)

        when:
        AWSCredentials credentials1 = provider.credentials
        AWSCredentials credentials2 = provider.credentials

        then:
        credentials1.AWSAccessKeyId == 'lichking'
        credentials1.AWSSecretKey == 'copperkey'
        credentials1.is(credentials2)
        1 * configService.secretLocalDirectory >> '/home/parzival/.lutus'
        1 * configService.accessIdFileName >> '.tombofhorrors'
        1 * configService.secretKeyFileName >> '.joust'
        1 * localFileReader.readFirstLine('/home/parzival/.lutus', '.tombofhorrors') >> 'lichking'
        1 * localFileReader.readFirstLine('/home/parzival/.lutus', '.joust') >> 'copperkey'
        0 * _
    }

    def 'should throw an exception if local credentials are not fully configured'() {
        provider = new LocalFilesCredentialsProvider(configService, localFileReader)
        ...
        then:
        1 * configService.secretLocalDirectory >> localDir
        1 * configService.accessIdFileName >> accessIdFile
        1 * configService.secretKeyFileName >> secretKeyFile
        thrown(AmazonClientException)
        0 * _
        credentials == null

        where:
        localDir     | accessIdFile | secretKeyFile
        null         | null         | null
        ''           | ''           | ''
        '/home/.aws' | '.accessid'  | ''
        '/home/.aws' | '.accessid'  | null
        '/home/.aws' | null         | '.secretkey'
        null         | '.accessid'  | '.secretkey'
    }
    ...
    def 'later threads should wait for first thread to finish fetching credentials, and should not fetch again'() {
        ...
        Closure<AWSCredentials> fetchWithCoordination = { String directory, String accessIdFileName,
        ...
        provider = new MultiThreadedLocalFilesCredentialsProvider(configService, localFileReader,
        ...
        2 * configService.secretLocalDirectory >> '/home/parzival/.lutus'
...
/**
 * A version of LocalFilesCredentialsProvider for multi-threaded unit testing, with a function that can be overridden
 ...
 */
class MultiThreadedLocalFilesCredentialsProvider extends LocalFilesCredentialsProvider {
     ...
     * @param fileReader the means for reading the contents of a local file
     ...
     */
    protected MultiThreadedLocalFilesCredentialsProvider(ConfigService configService, LocalFileReader fileReader,
    ...
    protected AWSCredentials fetchCredentials(String directory, String accessIdFileName, String secretKeyFileName) {
        fetchWithCoordination(directory, accessIdFileName, secretKeyFileName)
    }
}
